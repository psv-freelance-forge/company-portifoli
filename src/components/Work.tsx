        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
          {projects.map((project, i) => (
             <div key={i} className="group relative overflow-hidden rounded-[3rem] bg-zinc-900 border border-zinc-800 aspect-video md:aspect-4/5 lg:aspect-video cursor-pointer">
                ...
                <div className="absolute inset-x-6 md:inset-x-8 bottom-6 md:bottom-8 z-20 transition-transform duration-500 group-hover:-translate-y-4">
                   <h4 className="text-white text-2xl md:text-3xl font-black mb-2">{project.title}</h4>
                   <p className="text-zinc-400 font-bold text-xs md:text-sm tracking-widest uppercase mb-4 md:mb-6">{project.category}</p>
